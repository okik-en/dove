...
+ DBMS標準規格のアーキテクチャー、$3$層スキーマをサポートする意義として、データの独立性があり、二つの観点
  + 物理的
  + 論理的
  からの実装が議論される。この$2$つを詳しく具体的に説明せよ。
...
- データの物理配置変更
- インデックス構造の変更
+ これらの変更が、$3$層のどのスキーマ層に対応するか答えなさい
...
$B$木における検索のコストを考える。
$B$木の$1$つの節点には、$k$個ないし$2k$個のキーがある。
したがって、それぞれの節点は少なくとも#ana(1)個の子を持っている。
根節点だけは特別で、少なくとも#ana(2)個の子をもつ。

木の根の深さを$0$、全ての葉節点は同じ深さ$h$にあるとする。キーを与えて目的の節点を呼び出すためには、最低#ana(3)個、最大#ana(4)個の節点の呼び出しが必要である。
...
+ 探索キー値が$1, 2, 3, 4, 5, 6, 7$のレコードの最初は、空のヒープファイルに、
